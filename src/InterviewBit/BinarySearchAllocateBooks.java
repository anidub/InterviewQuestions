...
public class BinarySearchAllocateBooks {
...
//function to find minimum pages
	public static int findPages(int arr[], int m) {
		...
		while (start <= end) {
			// check if it is possible to distribute books by using mid is
			// current minimum
			int mid = (start + end) / 2;
			if (isPossible(arr, n, m, mid)) {
				...
				// if not possible means pages should be increased so update
				// start = mid + 1
				start = mid + 1;
		}
